...
const AddProductForm: React.FC<AddProductFormProps> = ({
  ...
  const handleGlobalAddComment = () => {
    console.log('Add Comments button clicked');
    if (formData.productPrices.length === 0) {
      showNotification('يرجى إضافة سعر أولاً.', 'warning', 'لا يوجد أسعار');
      return;
    }
    setShowAddCommentButtons((prev) => {
      console.log('showAddCommentButtons before:', prev);
      const newState = !prev;
      console.log('showAddCommentButtons after:', newState);
      return newState;
    });
  };

  const handleGlobalAddGroupProduct = () => {
    console.log('Add Group Products button clicked');
    if (formData.productPrices.length === 0) {
      showNotification('يرجى إضافة سعر أولاً.', 'warning', 'لا يوجد أسعار');
      return;
    }
    setShowAddGroupProductButtons((prev) => {
      console.log('showAddGroupProductButtons before:', prev);
      const newState = !prev;
      console.log('showAddGroupProductButtons after:', newState);
      return newState;
    });
  };
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <ActionButtons
            ...
            handleGlobalAddComment={handleGlobalAddComment}
            handleGlobalAddGroupProduct={handleGlobalAddGroupProduct}
          />
        </Grid>
